...
import { fetchAllProducts, updateProduct, deleteProduct as apiDeleteProduct } from "@/lib/admin-api";
...
export default function ProductsTable() {
  ...
  return (
    <div className="space-y-5">
                  ...
                  <tr key={product.id} className="table-row-hover border-b border-border/50 last:border-0">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-center gap-1.5">
                        <button onClick={() => setEditModal({ ...product })} className="p-2 rounded-lg text-primary bg-primary-lighter hover:bg-primary hover:text-white transition-all">
                          <Edit3 size={15} />
                        </button>
      ...
      {/* Edit Modal */}
      {editModal && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setEditModal(null)}>
          <div className="bg-white rounded-2xl w-full max-w-lg p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-lg font-bold text-text-primary">{t("products.editProduct")}</h3>
              <button onClick={() => setEditModal(null)} className="p-1.5 rounded-lg hover:bg-gray-100 text-text-muted"><XIcon size={18} /></button>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 mt-6 pt-5 border-t border-border">
              <button onClick={() => setEditModal(null)} className="px-4 py-2.5 rounded-xl border border-border text-sm font-medium text-text-secondary hover:bg-gray-50 transition-colors">{t("common.cancel")}</button>
